...
try {
	...
	$query_where = 'WHERE q.enable=1';

	// 전체 카운터 뽑기
	$stmt_count = $con -> prepare('SELECT COUNT(*) FROM questions AS q ' . $query_where);
	$stmt_count -> execute();
	...
	if ($data['total']) {
		...
		$stmt = $con -> prepare('SELECT q.*,IF(q.user_id,u.name,q.name) as name FROM questions AS q LEFT JOIN users AS u ON q.user_id=u.id ' . $query_where . ' ' . $query_order);
		$stmt -> execute();
